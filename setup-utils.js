...
const utilsDir = path.join(frontendSrcDir, 'utils');
const errorHandlerFile = path.join(utilsDir, 'errorHandler.js');
...
try {
  fs.writeFileSync(errorHandlerFile, errorHandlerContent, 'utf-8');
  console.log(`✓ Created file: ${errorHandlerFile}`);
  console.log('✓ Error handler utilities file created successfully!');
} catch (error) {
  console.error(`✗ Error creating errorHandler.js: ${error.message}`);
  process.exit(1);
}
